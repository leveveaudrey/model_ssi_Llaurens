...
bool sheltered_load_loci::selection_ID_multiplicative(sheltered_load_loci sll_mem, long a1, long a2)
{
 ...
 bool sheltered_load_loci::selection_ID_dominance(sheltered_load_loci sll_mem, long a1, long a2)
{
     bitset<100> b;
     long inc1=0, inc2=0;
     long r1,r2;
     char ch;
     double h=0.56, domsel=sel;
     ...
     b=sll_mem.vb[a1][inc1];
     b&=sll_mem.vb[a2][inc2];
   //  cout <<sll_mem.vb[a1][inc1][0]<<'\t'<<sll_mem.vb[a2][inc2][0]<<endl;cin >>ch;
     if (sll_mem.vb[a1][inc1][0]!=sll_mem.vb[a2][inc2][0]){domsel*=h;}
...
vb=vbtemp;
 ...
 void sheltered_load_loci::display(long repet,long gen)
 {
      char ch;
      
  ofstream myFile("load_ID.txt",ios::app);

if (! myFile) cout << "Impossible to open output file load_ID.txt (it must be closed)";

 
/*for (int i=0; i<vbf.size();i++) 
{myFile << "S" << i << '\t';
 for (int j=0;j<vbf[i].size();j++){for (int kk=0;kk<loci_number;kk++) myFile << vb[i][j][kk]; 
      myFile<< '\t' <<vbf[i][j] << '\t'<< vb[i][j].count()<<endl;}
...
for (int i=0; i<vbf.size();i++) 
{
myFile <<"repet" <<repet << '\t'<< "S" << i << '\t'<<"gen"<<gen;

for (int k=0;k<loci_number;k++) 
{ 
F[i][k]=0;
for (int j=0;j<vbf[i].size();j++)
if (j==0) F[i][k]+=(int)(vbf[i][j]*vb[i][j][k]);
   else  F[i][k]+=(int)(( vbf[i][j]-vbf[i][j-1])*vb[i][j][k]); 
 myFile<<'\t'<<F[i][k];
}
 if (!vbf[i].empty()) myFile<<'\t'<<vbf[i].back(); else myFile<<'\t' << 0;
...
/*for (int ii=0;ii<F.size();ii++) {cout << endl; for (int jj=0;jj<F[ii].size();jj++) 
cout << '\t' << F[ii][jj];}
cout << endl << endl;
for (int ii=0;ii<F.size();ii++)
for (int jj=0;jj<vbf[ii].size();jj++)
{for (int kk=0;kk<loci_number;kk++) cout << vb[ii][jj][kk]; 
      cout << '\t' <<vbf[ii][jj] << '\t'<< vb[ii][jj].count()<<endl;}
 cin >> ch;*/

}

void sheltered_load_loci::distribution_homozygotes_ID (long repet)
{
     ofstream myFile("distribution_homozygotes_ID.txt",ios::app);
if (! myFile) cout << "Impossible to open output file distribution_homozygotes_ID.txt (it must be closed)";

myFile << "repet" <<repet << '\t';

for (int i=0;i<dist_ho.size();i++) myFile<<dist_ho[i]<< '\t';   
...
long sheltered_load_loci::count()
{
     long ntot=0;
     
     for(long i=0;i<vb.size();i++)
     for(long j=0;j<vb[i].size();j++)
     ntot+=vb[i][j].count();
    
    return ntot; 
     }
     
long sheltered_load_loci::freq_count()
{
     long ntot=0;
     
     for(long i=0;i<vb.size();i++)
     for(long j=0;j<vb[i].size();j++)
    /* ntot+=vb[i][j].count()*vbf[i][j];*/
     
     
   if (j==0) ntot+=(vb[i][j].count()*vbf[i][j]);
   else  ntot+=(vb[i][j].count()*( vbf[i][j]-vbf[i][j-1])); 

   
    return ntot; 
     }

void sheltered_load_loci::mutation(alleles a, double mu,double eta, long N )
 { 
int i=0,j=0,k,nb_mut0_1,nb_mut1_0,l,ii;  
long inc0_1=0,inc1_0=0,r, nb1=freq_count();
string ch;
double rand_temp;
bitset<100> btemp;
if (nb1*eta>=1.0) nb_mut1_0= lround(nb1*eta);
    else nb_mut1_0=sto.Poisson(nb1*eta);
 
    
if ((2*N*loci_number-nb1)*mu>=1.0) nb_mut0_1= lround((2*N*loci_number-nb1)*mu);
    else nb_mut0_1=sto.Poisson((2*N*loci_number-nb1)*mu);
                                       //cout<<nb_mut<<endl;  
alleles atemp=a;



for (int i=1;i<a.size();i++) atemp[i]+=atemp[i-1];

while (inc0_1<nb_mut0_1 || inc1_0 <nb_mut1_0)
 { 
    i=0;
    j=0;
    rand_temp=al();          
    ...
    k=al.randInt(loci_number-1);          

    if ((vb[i][j][k]&& inc1_0 <nb_mut1_0)||(!vb[i][j][k]&&inc0_1 <nb_mut0_1)) 
     {
      btemp=vb[i][j];
      btemp[k]= !btemp[k];  
      if(vb[i][j][k])inc1_0++; else inc0_1++;
      ii=0;
      ...
      if (ii<vb[i].size()) for (int kk=j;kk<=ii-1;kk++) vbf[i][kk]--;
           ...
           {
             for (int kk=j;kk<vbf[i].size();kk++) vbf[i][kk]--; 
             vbf[i].push_back(vbf[i].back()+1); 
             vb[i].push_back(btemp);
             }                       
     }
    }
...
long sheltered_load_loci::distrib_all()
{       
    for(long i=0;i<vb.size();i++) 
             {
               while((int)(vb[i].size()-distrib[i].size())>0) distrib[i].push_back(0);     
               if (vb[i].size()!=0) distrib[i][vb[i].size()-1]++ ;    
             }
}

 
void sheltered_load_loci::ADistrib_file_out()    
{
     ofstream myFile("Allelic_Frequency_Distribution_ID.txt",ios::app);

     if (! myFile) cout << "Impossible to open output file Allelic_Frequency_Distribution_ID.txt (it must be closed)";

for (long k=0; k<vb.size();k++) {for (long l=0; l<distrib[k].size(); l++) myFile<< distrib[k][l]<<'\t'; myFile <<endl;    }
myFile<<endl;
...
void sheltered_load_loci::ADistrib_file_out(long a)    
{
     ofstream myFile("Allelic_Frequency_Distribution_ID.txt",ios::app);

     if (! myFile) cout << "Impossible to open output file Allelic_Frequency_Distribution_ID.txt (it must be closed)";

for (long l=0; l<distrib[a].size(); l++) myFile<< distrib[a][l]<<'\t'; myFile <<endl;  
}

void sheltered_load_loci::fixation_number(long nrepet, bool t)
{
     char ch;
        
        
       vector <vector <unsigned int > >  mat;
       vector <bitset<100> > stock;
        
        mat.resize(vb.size()); stock.resize(vb.size());
        for (int i=0;i<vb.size();i++) mat[i].resize(vb.size());
        ...
        for(int i=0;i<vb.size();i++)
                {
                 stock[i]=vb[i][0];
                    ...
                    stock[i] &= vb[i][j];
                    mat[i][i]=stock[i].count();
                  
               }   
               
         for(int i=0;i<vb.size();i++)
                for (int j=i+1; j<vb.size();j++)
                       mat[i][j]=(stock[i]&stock[j]).count();
                 
    ofstream myFile("fixation_number_ID.txt",ios::app);    
    if (t) myFile << "repet" <<nrepet << endl;
        for (int i=0;i<vb.size();i++)
            for (int j=i;j<vb.size();j++) myFile << mat[i][j] << '\t';
        myFile << endl;
        myFile.close(); 
        }
   ...
   for(long i=0;i<Gtemp.size();i++)
    {
        ...
        if (i==0) Gtemp[0][0]=G[0][0];
            ...
            {
                if (i!=0&&j==0) Gtemp[i][j]=Gtemp[i-1].back()+G[i][j]/2.0;
                else if (i!=j && j!=0) Gtemp[i][j]=Gtemp[i][j-1]+G[i][j]/2.0;
                else if (i==j && i!=0) Gtemp[i][j]=Gtemp[i][j-1]+G[i][j];
                
                
                }
    }

if (fabs((Gtemp.back()).back()-1.0)<10e-10) (Gtemp.back()).back()=1.0; else {cout << "Rounding problem in mutation procedure"; cin >> ch;}
    ...
    for(long i=0;i<nb_mut;i++)
            {
                        ...
                        m=n=0;
                        while (r>Gtemp[m][n])
                             {if (n<Gtemp.size()-1) n++; else {m++;n=0;}}
   ...
   for(long i=0;i<Gtemp.size();i++)
    {
     ...
     if (fabs((Gtemp.back()).back()-1.0)<10e-10) (Gtemp.back()).back()=1.0; else {cout << "Rounding problem in drift procedure : "<<Gtemp.back().back(); cin >> ch;}
  
    for(long ni=0;ni<N;ni++)
            {
                r=al();
                m=n=0;
                while (r>Gtemp[m][n]) {if (n<Gtemp.size()-1) n++; else {m++;n=m;}if(m>=Gtemp.size()) {cout << m << '\t'<< r << '\t' << (Gtemp.back()).back();}}
                G[m][n]+=1/(double) (N);
                G[n][m]=G[m][n];
                }
}

sheltered_load_loci genotypes::derive_ID_dominance(long N, sheltered_load_loci sll)
{
      vector<vector <double > > Gtemp;
    double r, survival;
    long m,n, ni=0;
    ...
    while(ni<N)
            {
                ...
                if (sll.selection_ID_dominance(sll_mem,m,n)) {G[m][n]+=1/(double) (N); G[n][m]=G[m][n];ni++;}
                }
...
void genotypes::sampling_without_replacement(long sample_size, long N)
{
     vector<vector <long > > Gtemp;
    long r;
    long m,n;
    string ch;
    Gtemp.resize(G.size());
   ...
   for(long i=0;i<Gtemp.size();i++)
    {
        Gtemp[i].resize(G.size());
        if (i==0) Gtemp[0][0]=lround(G[0][0]*N);
        for (long j=i;j<G.size();j++)
            {
                if (j!=i && j!=0) Gtemp[i][j]=Gtemp[i][j-1]+lround(G[i][j]*N);
                
                else if (j!=0) Gtemp[i][j]=Gtemp[i-1][G.size()-1]+lround(N*G[i][j]);
                G[i][j]=G[j][i]=0.0;
            }
        }
    ...
    for(long ni=0;ni<sample_size;ni++)
            {
                r=al.randInt(Gtemp.back().back()-1)+1;
                m=n=0;
                while (r>Gtemp[m][n]) {if (n<Gtemp.size()-1) n++; else {m++;n=m;}if(m>=Gtemp.size()) {cout << m << '\t'<< r << '\t' << (Gtemp.back()).back();cin >> ch;}}
                G[m][n]+=1.0/(double) (sample_size);
                G[n][m]=G[m][n];
                
   
        for (long j=n;j<G.size();j++) Gtemp[m][j]--;
        
         for (long i=m+1;i<Gtemp.size();i++) for (long j=i;j<G.size();j++)Gtemp[i][j]--;    
                }
}
...
for (long m=0;m<G.size();m++)
   ...
   if (verif) for (long m=0;m<G.size();m++) for (long n=0;n<=m;n++) temp+=G[m][n]; 
     if (verif && temp<10e-10) extinction=true;
     if (verif && temp> 10e-10 && fabs(temp-1.0)>10e-10) 
        for (long m=0;m<G.size();m++)                      
         for (long n=0;n<=m;n++) {G[m][n]/=temp; if (n!=m) G[n][m]=G[m][n];}
  }
